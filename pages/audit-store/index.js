const { goodsList, storeAudit, auditList } = require("../../utils/server.js")
...
Page({
  ...
  getData() {
    ...
    const data = { page, is_pass, type: 2 }
  ...
  auditHandler(e) {
    ...
    let { store_id, store_name } = list[index];
    const data = { is_pass: ispass, store_id, unionId }
    ...
    if (ispass == 0) {
      content = `是否将店铺【${store_name}】更改为待审核`
    } else if (ispass == 1) {
      content = `是否将店铺【${store_name}】更改为审核通过`
    } else if (ispass == 2) {
      content = `是否将店铺【${store_name}】更改为审核不通过`
    }
  ...
  jumpHandler(e) {
    ...
    const { store_id } = list[index];

    wx.navigateTo({
      url: `/pages/stores/details/details?store_id=${store_id}`,
    })
